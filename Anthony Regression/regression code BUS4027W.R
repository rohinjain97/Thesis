...
shapiro.test(Exam) #not normally distributed p=0.03658
qqPlot(Exam)
...
#model 4: demog is most significant
...
AIC(fit_normal_2_1,fit_normal_2_2,fit_normal_2_3,fit_normal_2_4) #model 2:regstiered programme is best
anova(fit_normal_1_4,fit_normal_2_2,test = "Chisq") #add registered programme

fit_normal_3_1 = glm(Exam~DEMOG+registered_program + registered_program*DEMOG, family = gaussian(link = ))

anova(fit_normal_2_2,fit_normal_3_1,test = "Chisq")
AIC(fit_normal_2_2,fit_normal_3_1) #do not add an interaction affect

fit_normal_3_2 = glm(Exam~DEMOG+registered_program + protest_indicator, family = gaussian(link = ))
fit_normal_3_3 = glm(Exam~DEMOG+registered_program + Gender, family = gaussian(link = ))
fit_normal_3_4 = glm(Exam~DEMOG+registered_program + Deferred, family = gaussian(link = ))

AIC(fit_normal_3_2,fit_normal_3_3,fit_normal_3_4)
anova(fit_normal_2_2,fit_normal_3_3,test = "Chisq") #do not add gender
#stop at 2.2
#final model is DEMOG and registered program

fit_normal_protest_indicator = glm(Exam~ protest_indicator, family = gaussian(link = ))

#comment
summary(fit_normal_2_2)
